...
To analyze a classifier $c_l$ we are mainly interested in the relation between the thresholds $g_l^+$ and $g_l^-$ and the true positive rate (\TPR) and the false positive rate (\FPR). For each classifier we obtain two ROC curves (receiver operating characteristics):
\begin{itemize}
...
one atom $A_i$ producing a true positive, i.e., $A_i$ correctly identified a human. In both cases $\Phi$ does not belong to allocation bases for false negatives.% Again, this consideration emphasizes the importance to maximize the true positive rate.
